...
class RentalModelTests(TestCase):
    ...
    def setUp(self):
        ...
        self.bicycle = Bicycle.objects.create(
            model='Test Bicycle', price=Decimal('10.00')
        )
    ...
    def test_rental_permission_to_return_not_returned(self):
        rental = Rental.objects.create(
            bicycle=self.bicycle, renter=self.user, is_returned=False
        )
        rental.return_bicycle()
        self.assertTrue(rental.is_returned)
...
class RentalAdminTest(TestCase):
    ...
    def setUp(self):
        ...
        self.bicycle = Bicycle.objects.create(
            model='Test Bicycle', price='10.00'
        )
        self.rental = Rental.objects.create(
            bicycle=self.bicycle, renter=self.user, start_time=timezone.now()
        )
    ...
    def test_readonly_fields(self):
        ...
        self.assertEqual(
            self.rental_admin.readonly_fields, expected_readonly_fields
        )
...
class RentalSerializerTest(TestCase):
    ...
    def test_rental_serializer_update(self):
        ...
        serializer = RentalSerializer(
            instance=rental, data=data, partial=True, context={'request': None}
        )
